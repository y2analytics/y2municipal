...
test_that("methodology warns and continues with missing duration column", {
  dataset <- tibble::tibble(
    StartDate = c(
      lubridate::mdy_hms('Jan 1 2022 10:00:00'),
      lubridate::mdy_hms('Feb 1 2022 10:00:00')
    ),
    EndDate = c(
      lubridate::mdy_hms('Jan 1 2022 10:00:10'),
      lubridate::mdy_hms('Feb 1 2022 10:00:30')
    )
  )
  expect_snapshot(methodology(dataset, 100000))
})
...
test_that("get_field_dates formats correctly", {
  ...
  expect_equal(
    get_field_dates(dataset, StartDate, EndDate),
    'Fielded: Jan 1 - Feb 1'
  )
})
...
test_that("get_field_dates returns NULL with warning when columns missing", {
  ...
  expect_warning(
    result <- get_field_dates(dataset, StartDate, EndDate),
    "StartDate"
  )
  expect_null(result)
})
